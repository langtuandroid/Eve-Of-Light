...
namespace NeoFPS
{
    ...
    public class WieldableLaserPointer : MonoBehaviour, IWieldableFlashlight, ILaserPointer, INeoSerializableComponent
    {
        ...
        private static List<ILaserPointerWatcher> s_Watchers = new List<ILaserPointerWatcher>();
        ...
        protected void Awake()
        {
            ...
            // Register with any watchers
            var t = transform;
            while (t != null)
            {
                // Get any laser watchers
                t.GetComponents(s_Watchers);

                // Register the laser with them
                for (int i = 0; i < s_Watchers.Count; ++i)
                    s_Watchers[i].RegisterLaserPointer(this);

                // Clear the watchers list
                s_Watchers.Clear();

                // Break out at the firearm itself (shouldn't be any on the character)
                if (t.GetComponent<IModularFirearm>() != null)
                    break;
        ...
        protected void OnDestroy()
        {
            ...
            {
                // Get any laser watchers
                GetComponents(s_Watchers);

                // Register the laser with them
                for (int i = 0; i < s_Watchers.Count; ++i)
                    s_Watchers[i].UnregisterLaserPointer(this);

                // Break out at the firearm itself (shouldn't be any on the character)
                if (GetComponent<IModularFirearm>() != null)
                    break;
