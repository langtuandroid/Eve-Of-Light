...
namespace NeoCC
{
    ...
    public class NeoCharacterController : MonoBehaviour, INeoCharacterController, INeoCharacterVariableGravity, INeoSerializableComponent
    {
        ...
        const float k_GroundCheckOvershoot = 0.002f;
        ...
        public float slopeLimit
        {
            ...
            set { m_SlopeLimit = Mathf.Clamp(value, k_ClampMaxSlopeLow, 90f - m_WallAngle); }
        }
        ...
        public void Initialise ()
        {
            ...
            // Get the initial grounding check distance
            // (this will also be done every frame at runtime in the editor in case the properties are changed)
            GetGroundingCheckDistance();

            // Ignore own transform
            m_IgnoreTransforms.Add(transform);
        ...
        bool IsHeightRestricted(float h, float fromNormalisedHeight, out float offset)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (PhysicsExtensions.SphereCastNonAllocSingleFiltered(
                    ray,
                    r - m_SkinWidth,
                    out m_Hit,
                    m_IgnoreTransforms,
        ...
        {
            // Skip if time is effectively zero (leas
            ...
            if (!ignorePlatforms && platform != null && inheritPlatformYaw)
            {
                ...
                Quaternion rotationDiff = GetYawRotation(platformRotation);
                m_TargetRotation = rotationDiff * m_TargetRotation;
            }
        ...
        void MoveLoop ()
        {
            ...
            while (GetMoveVector(out move))
            {
                ...
                if (collisionsEnabled && PhysicsExtensions.CapsuleCastNonAllocSingleFiltered(
                    ...
                    ))
                {
                            ...
                            default:
                                {
                                    ...
                                    Vector3 flattenedNormal = Vector3.ProjectOnPlane(m_Hit.normal, m_CurrentUp).normalized;

                                    // Horizontal wall deflection
                                    if (horizontalMagnitude > k_MinMoveDistance)
                                    {
                                        // Get the up slope amount
                                        float upSlopeHeading = -Vector3.Dot(horizontal.normalized, flattenedNormal);

                                        // If trying to move up slope, deflect
                                        if (upSlopeHeading > 0f)
                                            newMove = Vector3.ProjectOnPlane(horizontal, flattenedNormal);
                                        else
                                            newMove = horizontal;
                                    }
                                    else
                                    ...
                                    // Push the controller slightly from the wall
                                    newMove += flattenedNormal * m_SkinWidth;
                    ...
                    if (fireCollisionEvent)
                    {
                        ...
                        INeoCharacterController hitCharacter = null;
                        if (pushCharacters)
                            m_Hit.collider.TryGetComponent(out hitCharacter);
        ...
        void GetGroundingCheckDistance()
        {
            float inner = radius - m_SkinWidth;
            m_GroundingCheckDistance = Mathf.Sqrt(radius * radius - inner * inner) + k_GroundCheckOvershoot;
        }
        ...
        void CheckGrounding ()
        {
            ...
            {
                hitGround = Physics.SphereCast(GetCapsuleBottom(), radius - m_SkinWidth, -m_CurrentUp, out m_Hit, m_GroundingCheckDistance, collisionMask, QueryTriggerInteraction.Ignore);
                if (hitGround)
                {
                    Vector3 localHitNormal = m_InverseRotation * m_Hit.normal;
                    ...
                    // Check against overshoots
                    if (m_Hit.distance > Mathf.Lerp(m_GroundingCheckDistance + k_GroundCheckOvershoot, m_SkinWidth + k_GroundCheckOvershoot, localHitNormal.y))
                        hitGround = false;
                }
                else
                    hitGround = Physics.Raycast(GetCapsuleBottom(), -m_CurrentUp, out m_Hit, radius + 0.05f, collisionMask, QueryTriggerInteraction.Ignore); // Hacky workaround for Unity physics bug where sphere casts against edges fail
            }
        ...
        bool CheckIgnoreTransform(Transform t)
        {
            return IsPartOfHeirarchy(t) || m_IgnoreTransforms.Contains(t);
        }
...
#endif
                    totalDepenetration = ConstrainDepenetrationToGround(totalDepenetration);
        ...
        Vector3 ConstrainDepenetrationToGround(Vector3 depenetration)
        {
            if (isGrounded)
            {
                float groundNormalDepenetration = Vector3.Dot(depenetration, groundNormal);
                if (groundNormalDepenetration < 0f)
                    depenetration -= groundNormalDepenetration * groundNormal;
            }
        ...
        /// <param name="ignoreTilt">Should the teleport rotation be modified to maintain the character's up vector?</param>
        public void Teleport(Vector3 position, Quaternion rotation, bool relativeRotation = true, bool ignoreTilt = false)
        {
            ...
            if (relativeRotation)
            {
                ...
                if (!ignoreTilt)
                {
                    m_TargetUp = rotation * m_TargetUp;
                    m_CurrentUp = rotation * m_CurrentUp;
                    m_StartRotation = m_TargetRotation;
                }
            }
            else
            {
                ...
                if (!ignoreTilt)
                {
                    m_TargetUp = relative * m_TargetUp;
                    m_CurrentUp = relative * m_CurrentUp;
                    m_StartRotation = m_TargetRotation;
                }
            }
            ...
            if (ignoreTilt)
            {
                var tiltCorrection = Quaternion.FromToRotation(m_TargetRotation * Vector3.up, m_CurrentUp);
                m_TargetRotation *= tiltCorrection;
                m_StartRotation = m_TargetRotation;
            }
        ...
        Quaternion GetYawRotation(Quaternion rotation)
        {
            Vector3 ra = new Vector3(rotation.x, rotation.y, rotation.z);
            Vector3 p = Vector3.Project(ra, m_LocalTransform.up);
        ...
        public void SetIgnoreRigidbody(Rigidbody rb)
        {
            var rbt = rb?.transform;
            if (!m_IgnoreTransforms.Contains(rbt))
                m_IgnoreTransforms.Add(rbt);
        }

        public void ResetIgnoreRigidbody(Rigidbody rb)
        {
            var rbt = rb?.transform;
            if (rbt != m_LocalTransform)
                m_IgnoreTransforms.Remove(rb.transform);
        }
