...
namespace NeoFPSEditor
{
    public static class NeoFpsEditorGUI
    {
        ...
        delegate void ProcessDroppedObject(SerializedProperty prop, UnityEngine.Object dropped);

        private static SerializedProperty s_PendingBrowserProperty = null;

        static void AssignDroppedObject(SerializedProperty prop, UnityEngine.Object dropped)
        {
        ...
        public static GameObject GameObjectInHierarchyField(GUIContent label, SerializedProperty prop, Transform root, GameObjectFilter filter, bool allowRoot = true)
        {
            ...
            else
            {
                ...
                if (DrawCustomObjectField<GameObject>(label, prop, true, ProcessObjectInHierarchy))
        ...
        public static T ComponentInHierarchyField<T>(GUIContent label, SerializedProperty prop, Transform root, bool allowRoot = true) where T : class
        {
            ...
            else
            {
                ...
                // Show the object field
                if (DrawCustomObjectField<T>(label, prop, true, ProcessObjectInHierarchy))
        ...
        static void ProcessObjectInHierarchy(SerializedProperty prop, UnityEngine.Object dropped)
        {
            // Null is easy
            if (dropped == null)
                prop.objectReferenceValue = null;
            else
            {
                ...
                if (stage != null)
                {
                    ...
                    var referenceGameObject = dropped as GameObject;
                    ...
                    if (isReferenceStaged && !isInspectingStaged)
                    {
                        ...
                        if (corresponding != null)
                        {
                            ...
                            if (dropped is GameObject)
                                dropped = NeoFpsEditorUtility.GetRelativeGameObject(prefabRoot, corresponding.gameObject, referenceGameObject);
                            else
                                dropped = NeoFpsEditorUtility.GetRelativeComponent(prefabRoot, corresponding.gameObject, dropped as Component);
                    ...
                    else
                    {
                        if (!isReferenceStaged && isInspectingStaged)
                        {
                            ...
                            var refComp = dropped as Component;
                            if (refComp != null)
                            {
                                ...
                                {
                                    dropped = inspectingComponents[index];
                                    Debug.Log("Correcting for prefab-stage. Be aware that the prefab open for editing is not the same object as the prefab in the project browser.");
                                }
                            }
                            else
                            {
                                ...
                                dropped = stage.prefabContentsRoot;
                                Debug.Log("Correcting for prefab-stage. Be aware that the prefab open for editing is not the same object as the prefab in the project browser.");
                            }
                        }
                        prop.objectReferenceValue = dropped;
        ...
        static bool DrawCustomObjectField<T>(GUIContent label, SerializedProperty prop, bool allowSceneObjects, ProcessDroppedObject processDroppedObject)
        {
            bool result = false;
            ...
            using (var scope = new EditorGUILayout.HorizontalScope())
            {
                ...
                var reference = EditorGUI.ObjectField(fullRect, GUIContent.none, original, typeof(T), allowSceneObjects);
                if (reference != original)
                    processDroppedObject(prop, reference);
            }
        ...
        public static GameObject GameObjectInHierarchyField(Rect rect, SerializedProperty prop, Transform root, GameObjectFilter filter, bool allowRoot = true)
        {
            ...
            else
            {
                ...
                // Show the object field
                if (DrawCustomObjectField<GameObject>(rect, prop, true, ProcessObjectInHierarchy))
                    ObjectHierarchyBrowser.GetChildObject(root, allowRoot, OnGameObjectPicked, OnObjectPickingCancelled, filter);
        ...
        public static T ComponentInHierarchyField<T>(Rect rect, SerializedProperty prop, Transform root, bool allowRoot = true) where T : class
        {
            ...
            else
            {
                ...
                // Show the object field
                if (DrawCustomObjectField<T>(rect, prop, true, ProcessObjectInHierarchy))
                    ObjectHierarchyBrowser.GetChildObject(root, allowRoot, OnComponentPicked<T>, OnObjectPickingCancelled);
        ...
        static bool DrawCustomObjectField<T>(Rect rect, SerializedProperty prop, bool allowSceneObjects, ProcessDroppedObject processDroppedObject)
        {
            bool result = false;
            ...
            // Show object field
            var original = prop.objectReferenceValue;
            var reference = EditorGUI.ObjectField(rect, GUIContent.none, original, typeof(T), allowSceneObjects);
            if (reference != original)
                processDroppedObject(prop, reference);
        ...
        public static GameObject PrefabField(GUIContent label, SerializedProperty prop, GameObjectFilter filter)
        {
            ...
            // Show the object field
            if (DrawCustomObjectField<GameObject>(label, prop, false, AssignDroppedObject))
        ...
        {
            // Show the object field
            if (DrawCustomObjectField<T>(label, prop, false, AssignDroppedObject))
        ...
        public static GameObject PrefabField(Rect rect, SerializedProperty prop, GameObjectFilter filter)
        {
            ...
            // Show the object field
            if (DrawCustomObjectField<GameObject>(rect, prop, false, AssignDroppedObject))
                ProjectHierarchyBrowser.GetPrefab(OnGameObjectPicked, OnObjectPickingCancelled, filter);
        ...
        {
            // Show the object field
            if (DrawCustomObjectField<T>(rect, prop, false, AssignDroppedObject))
                ProjectHierarchyBrowser.GetPrefabWithComponent<T>(OnObjectPicked, OnObjectPickingCancelled);
        ...
        public static Texture2D mgKeyFoundTexture
        {
            get
            {
        ...
        public static Texture2D mgKeyNotFoundTexture
        {
            get
            {
        ...
        public static bool RequiredStringField(SerializedProperty prop)
        {
            ...
            else
            {
                EditorGUILayout.BeginHorizontal();
                DrawRequiredPrefixLabel(prop, valid);
                EditorGUILayout.DelayedTextField(prop, GUIContent.none);
                EditorGUILayout.EndHorizontal();
            }
            return valid;
        }
