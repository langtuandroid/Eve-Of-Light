...
namespace NeoFPS
{
	...
	public class InputCharacterMotion : CharacterInputBase
    {
        ...
        private struct MotionGraphInput
        {
            ...
            public MotionGraphParameter parameter { get; private set; }

            public void Initialise (MotionGraphContainer motionGraph)
            {
                switch (type)
                {
                    case ParameterType.Trigger:
                        parameter = motionGraph.GetTriggerProperty(parameterKey);
                        break;
                    case ParameterType.Switch:
                        parameter = motionGraph.GetSwitchProperty(parameterKey);
                        break;
                }
            }
        }
        ...
        protected override void OnLoseFocus()
        {
            ...
            if (m_SprintHoldProperty != null)
                m_SprintHoldProperty.Hold(false);
            if (m_CrouchHoldProperty != null)
                m_CrouchHoldProperty.Hold(false);
        ...
        protected override void UpdateInput()
        {
            ...
            if (allowMoveInput)
            {
                ...
                // Custom Motion Graph Inputs
                for (int i = 0; i < m_CustomInputs.Length; ++i)
                {
                    if (m_CustomInputs[i].button != FpsInputButton.None && !string.IsNullOrWhiteSpace(m_CustomInputs[i].parameterKey))
                    {
                        switch (m_CustomInputs[i].type)
                        {
                            case ParameterType.Trigger:
                                {
                                    var parameter = m_CustomInputs[i].parameter as TriggerParameter;
                                    if (parameter != null && GetButtonDown(m_CustomInputs[i].button))
                                        parameter.Trigger();
                                }
                                break;
                            case ParameterType.Switch:
                                {
                                    var parameter = m_CustomInputs[i].parameter as SwitchParameter;
                                    if (parameter != null)
                                        parameter.on = GetButton(m_CustomInputs[i].button);
                                }
                                break;
                        }
                    }
